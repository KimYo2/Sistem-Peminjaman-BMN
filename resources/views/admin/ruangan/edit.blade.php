...
@section('content')
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm p-6 sm:p-8 transition-colors">
            ...
            <div class="mb-6">
                <h1 class="text-xl font-bold text-slate-800 dark:text-white">Edit Ruangan</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ $ruangan->kode_ruangan }} — {{ $ruangan->nama_ruangan }}</p>
            </div>

            <form action="{{ route('admin.ruangan.update', $ruangan->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
